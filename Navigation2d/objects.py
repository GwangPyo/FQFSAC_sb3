import numpy as np
import copy
from .util import normalize_position, denormalize_position
...
class Obstacle(object):
    def __init__(self, world, min_speed, max_speed, move_range, mode=0, prob_uncertain=0.1):
        self.mode = mode
        self.world = world
        ...
        self.prob_uncertain = prob_uncertain
    ...
    def set_linear_vel(self, init=False):
        ...
        if self.direction == 0:
            return
        mode = np.random.choice(["fast", "slow"], p=[self.prob_uncertain, 1 - self.prob_uncertain])
        if mode == "slow":
